...
**Added 2026-08-31, retroactively.** This section did not exist, which is why
`npm run sdd:preclear` failed on this slice — it enforces these four headings, so its absence is
direct evidence that the Document (build notes) stage was never run here. The headings above were
renamed to the template at the same time, so the check stops failing on every later slice; the
content is untouched apart from the two factual corrections above.

What was shaky, written now rather than then:

- **Neither boundary was ever rendered by anything.** No test mounted them and no validation row
  triggered a throw. The evidence recorded for the slice was a `200 OK` on a healthy page, which
  exercises no error boundary at all. Tracked as **#467**, closed by
  `specs/2026-08-31-error-boundary-gaps/`.
- **`validation.md`'s V2 was unexecutable as written.** It asked for `global-error.tsx` to be
  verified under `npm run dev`; Next substitutes `global-error.tsx` only in a production build, so
  the dev overlay would have owned that screen. `CLAUDE.md` independently forbids `next dev` for
  anything DB-touching, which both group layouts are.
- **Where the boundaries actually sit in the layout tree was assumed, not checked.** That assumption
  is what produced #478.
- **The colour values were never checked against the token audit.** That is #479.
