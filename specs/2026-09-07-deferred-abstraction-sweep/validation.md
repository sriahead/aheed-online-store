...
| R4  | Unit | `npx vitest run tests/radius-scale.test.ts` exits 0. Then prove it bites **in the direction that matters**: temporarily reintroduce a single `rounded-md` class into any `.tsx` file under `components/`, re-run, confirm it FAILS naming that file and the colliding pair, then revert. |
...
| R17 | Integration | `grep -oE 'name="brand[A-Za-z]+"' components/staff/StorefrontConfigForm.tsx \| sort -u` prints exactly eight distinct names, matching the eight schema columns. |
...
| R20 | Unit | `sed -n '/^model Theme /,/^}/p' prisma/schema.prisma` shows a name field and eight `String` brand columns and contains no `Json`. |
| R21 | Unit | The same `sed` output contains no `vendorId` field and no `Vendor` relation. `npx vitest run tests/repository-vendor-scoping.test.ts` exits 0. |
...
| R23 | Integration | `grep -n "theme\|Theme" lib/vendor-theme.ts` shows no join or `Theme` lookup. Then live: under `npm run preview` select a theme on `/staff/storefront`, then edit ONE colour field to a different value and save; re-query `VendorBranding` and confirm that one column differs from the theme's value while the other seven still match it. |
...
| R35 | Regression | `git diff origin/staging...HEAD -- prisma/schema.prisma` shows no changed line inside the `OrderItem` model block. |
