...
| R9 | Unit | `grep -rn "Deliberately NO quality job" .` returns no matches. `grep -n "protect-staging" .github/workflows/deploy-staging.yml` returns at least one line, and the comment around it also mentions `quality.yml`. |
