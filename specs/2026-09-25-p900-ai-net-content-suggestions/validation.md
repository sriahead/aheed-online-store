...
| R32 | Unit | `npm run kms:validate` exits 0 with 0 failing. `npm run kms:build-index`, then `grep -n "p900-ai-net-content-suggestions" ARTIFACT_INDEX.md` matches (the row links the file by path; `build-index.ts` never renders a front-matter `id` string literally, confirmed against other specs' entries too — the original pattern named the `id` value itself and could never match). Run `npm run kms:assemble:internal`, then `cd kms/site-internal` and `npx next build --webpack`. Read its real exit status, not a piped one: 0. |
