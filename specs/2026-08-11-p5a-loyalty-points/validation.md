...
| R24 | `ls prisma/migrations/` shows exactly one new directory vs `origin/staging`. In its `migration.sql`: `grep -c "DROP" ` prints `0`, `grep -c "SET NOT NULL"` prints `0`, and `grep -n "ALTER TYPE"` matches nothing (the new enum is a `CREATE TYPE`). `grep -c "CREATE TABLE"` prints `3`. |
