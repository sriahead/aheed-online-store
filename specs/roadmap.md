...
| 2026-08-09 | **ADR-004 slice 3c** (#74): **data-driven auth cookie scoping** — the last multi-tenancy gate before P3. `getAuth()`'s `baseURL`/`trustedOrigins`/cookie domain resolve per request from the host + `VendorDomain` (`lib/auth-origin.ts`); **host-only (isolated) sessions by default** (no shared subdomain family exists — Aheed and SriMart are on distinct hosts), with a config-gated `AUTH_COOKIE_FAMILY_DOMAIN` (unset today) that arms future parent-domain family SSO with no code change. ADR-004 breadcrumb records the topology-vs-decision-4 reconciliation. **ADR-004 slice sequence complete — P3 (cart & checkout) unblocked.** | Auth made per-vendor before P3 adds order/checkout query surface |
