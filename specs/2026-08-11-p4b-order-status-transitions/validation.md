...
| R23 | With F6… seeded past one page, confirm the staff list method paginates by **cursor**, not by offset: its query passes `cursor` + `take`, and any `skip:` present is Prisma's `skip: 1` cursor-exclusion idiom, never a computed page offset. Fetch page 1, collect its order numbers, follow the rendered next-page link, collect page 2's. The two sets are disjoint, their union contains every actionable Aheed order the DB holds (`SELECT "orderNumber" FROM "Order" WHERE "vendorId"=<aheed> AND status IN ('CONFIRMED','OUT_FOR_DELIVERY')`), and no order number repeats. **Corrected 2026-08-11, at Validate.** This row originally required `grep -n "OFFSET\|skip:" lib/repositories/orders.ts` to show **no** `skip:` in the staff list method. That check is wrong: `skip: 1` is Prisma's standard keyset idiom for "skip the cursor row itself" and is exactly what a correct keyset implementation contains — it already ships in P4a's `listForUser` and P2b's `products.ts`, and `specs/architecture.md` endorses it explicitly ("Prisma `cursor` + `take` implements this directly"). The literal grep would fail a correct implementation and pass one that dropped the cursor entirely. Same class as R5/R9's greps here and R5/R27's in P4a: the requirement's real property cannot be expressed as the absence of a token. The union-and-disjointness check below is what actually proves R23, and it was verified live across a real 20/6 page boundary. |
