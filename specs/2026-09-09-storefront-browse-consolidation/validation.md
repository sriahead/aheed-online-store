...
| R7  | Regression   | `test ! -f components/product/CategoryDrillDown.tsx` exits 0, and `grep -rn "CategoryDrillDown" app components features lib \| grep -v "runbook/docs.ts"` returns nothing. |
| R8  | Integration  | `grep -n "listTopLevel()" "app/(storefront)/search/page.tsx"` returns nothing. Then compare category call counts against the base branch: `grep -c "categoryRepo\.\|getCategoryRepository()" "app/(storefront)/search/page.tsx"` is less than or equal to `git show origin/staging:"app/(storefront)/search/page.tsx" \| grep -c "categoryRepo\.\|getCategoryRepository()"`. |
...
| R10a | Integration | In `search.html`, each of the four collection labels appears exactly once: `for L in "All products" "New Arrivals" "Featured Products" "Value Bundles"; do echo -n "$L: "; grep -o "$L" search.html \| wc -l; done` prints `1` for every label. A count of `2` means the nav was placed inside `FilterPanel`'s duplicated branches. |
...
| R15 | Security     | `curl -s -o /dev/null -w "%{http_code}" -H "Host: <seeded host>" "http://127.0.0.1:8787/search?category=definitely-not-a-real-slug"` prints `200`, and the body contains no filter chip for that value (`grep -c 'definitely-not-a-real-slug' body.html` returns `0` outside the select's own markup). |
