...
| R14 | Two parts. **(a)** Seed every constituent of one bundle with `originalPrice` null, fetch `/categories`, extract only that bundle's card markup, and confirm it contains exactly one price string and no `line-through`/`<s>`/`<del>` element and no "Save" text. **(b)** Set `originalPrice > basePrice` on one constituent of that same bundle and re-fetch — that bundle's own card markup still shows exactly one price (the derived total), with no per-constituent price, no badge, and no "Save" text anywhere in the card; separately, fetch a page where `ProductCard` renders that same product outside a bundle (e.g. `/products/[slug]` or `/search`) and confirm its badge still appears there, unaffected by this slice. **A blanket `/save/i` grep over the whole page fails part (b) by construction and must not be used** — target the bundle card's own markup, not the page's. **(Corrected at `/validate` 2026-08-25 — this row previously required the constituent's badge to appear inside the bundle card, contradicting part (a); see `requirements.md` R14.)** |
