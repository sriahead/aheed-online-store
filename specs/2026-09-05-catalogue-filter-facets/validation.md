...
| R14 | Regression | `grep -rn "getAvailableSpecialities\|AvailableSpecialities\|SpecialityContext\|availableSpecialities" lib/ app/ components/ scripts/ tests/` returns no matches. Then `npm run typecheck` exits 0, proving every call site moved rather than being deleted. |
