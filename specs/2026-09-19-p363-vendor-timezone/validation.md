...
| R6  | Unit | `TZ=UTC npx vitest run tests/local-datetime.test.ts` and `TZ=PST8PDT npx vitest run tests/local-datetime.test.ts` both exit 0 with identical assertion counts, and the file contains a case asserting `calendarDayInZone(new Date("2026-09-18T23:30:00Z"), "Europe/London") === "2026-09-19"`. **Use slash-free `TZ` values on Windows** — measured 2026-09-19, a value containing a slash (`TZ=Pacific/Auckland`) never reaches `process.env.TZ` there and the run silently uses the system zone, so the second run proves nothing. `UTC`, `PST8PDT` and `EST5EDT` all work; on the Linux CI runner any IANA name works. Confirm the two runs really differed before trusting them. |
...
| R19 | Unit | `grep -n 'toISOString' components/checkout/SlotPicker.tsx` returns **only a comment line** describing the old behaviour — no executable occurrence — and the hidden `fulfilmentDate` input's `value` is `{selectedDay}`, the `YYYY-MM-DD` string. The surviving comment hits are deliberate: they record the defect, so treat a hit inside a `/* */` or `//` block as a pass, not a failure. |
| R20 | Unit | `grep -n -A 5 'Intl.DateTimeFormat' components/checkout/SlotPicker.tsx` shows the single label formatter carrying `timeZone: "UTC"` — explicitly UTC, not the vendor zone, because UTC midnight of a day rendered in a negative-offset zone is the previous evening. |
...
| R23 | Unit | `grep -n 'zoneWallClock' components/checkout/SlotPicker.tsx` shows the express check using it, still inside the `useEffect` at the top of the component. `grep -n 'now.getDay()\|now.getHours()\|setHours(0' components/checkout/SlotPicker.tsx` returns **only comment lines** recording the old behaviour, no executable ones — same reading rule as R19. |
...
| R25 | E2E | **The row that proves `#811`.** First establish the check discriminates: against **deployed staging** (pre-fix code), in Chrome with DevTools → Sensors → timezone overridden to a UTC+1 zone, load `/checkout` with a cart and pick a date whose weekday has slots — the list shown is the **previous** day's. Then, with `npm run preview` running locally (Worker runtime, UTC) on the slice branch: (a) `TZ=UTC npx tsx specs/2026-09-19-p363-vendor-timezone/verify-vendor-timezone.ts --slots` and the same command under `TZ=PST8PDT` both report the same `SUBMITTED DAY` and pass every weekday row — the script prints an `AMBIENT ZONE` line with the effective offset, and the two runs must show **different offsets** or they proved nothing (see R6 on Windows and slashes); (b) repeat the Chrome check with the same UTC+1 override and confirm the slot list is now the picked day's; (c) complete the order and confirm the stored `Order.fulfilmentDate` reads exactly `T00:00:00.000Z` for the picked date. |
