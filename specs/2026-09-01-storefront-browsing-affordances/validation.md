...
| R2  | Regression | `git diff "$(git merge-base origin/staging HEAD)"..HEAD -- lib/repositories/products.ts` touches only the `list()` interface docstring (R18) — no change to `searchProducts` or any other executable line. The `ProductRepository` interface in that file still declares both `list` and `search`. |
...
| R18 | Regression | `grep -rn "nothing searched yet" lib/` returns no hit. Read the `list()` docstring on the `ProductRepository` interface in `lib/repositories/products.ts` (**not** `lib/products-service.ts` — see R18's correction note): it states that `/search` branches to `list()` for its browse mode while `search()` keeps its own empty-query guard. |
