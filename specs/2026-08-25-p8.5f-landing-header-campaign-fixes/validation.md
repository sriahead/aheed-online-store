...
| R13 | `Get-Content proxy.ts` — a default-exported function returning `NextResponse.next({ request: { headers } })` with `x-pathname` set on those headers, a `config` export whose `matcher` excludes `_next/static`, `_next/image` and `favicon.ico`, and no `export const runtime`. |
...
| R16 | Load `/` in a browser under preview with DevTools → Network. Every `/_next/static/...` request returns `200` and the page's CSS/JS applies (an unstyled page means the matcher is wrong). |
