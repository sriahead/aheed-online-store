...
| R2  | Integration | `node -e "const p=require('./package.json');console.log(p.scripts['kms:check-generated'])"` prints a command running `kms/scripts/check-generated.ts`. `grep -n "GENERATED_ARTIFACTS" kms/scripts/check-generated.ts` shows it imported and used. Then confirm the list is not restated **in code**: `grep -vE '^\s*(\*\|//\|/\*)' kms/scripts/check-generated.ts \| grep -cE "ARTIFACT_INDEX\.md\|runbook/docs\.ts"` returns `0`. **The comment-stripping is load-bearing** — the file's doc comment legitimately names both paths when explaining the defect, and a naive grep over the whole file returns 3 and fails a correct implementation. This is the same reason `tests/repository-client-injection.test.ts` is AST-based rather than a grep (CLAUDE.md, repository layer). |
