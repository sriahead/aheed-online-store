...
| R22 | `docs/nfr-baseline.md` records the re-run: for each pattern exercised, the mutations attempted, the interval, the concurrency, and the count at which failure occurred (or "none"). Reproduce it if you can — drive `Add to cart` on a staging product page ~20 times at ~1.1s intervals (the original observation's method), or issue the equivalent server-action POSTs headlessly. The action id is recoverable from staging's own client bundle: fetch a product page, collect its `/_next/static/chunks/*.js`, and grep for `createServerReference("<40-hex>"…"addToCart")` — no local build needed. A different outcome from the recorded one is a finding to report, not a fail; record what you saw. |
| R22a | `grep -n "attribut" docs/nfr-baseline.md` — the document says which failures were and were not attributed, and names the post-`deploy-staging` Workers Logs as what server-side attribution needs. Confirm it does **not** assert a server-side cause for a client-observed connection failure without that evidence. |
