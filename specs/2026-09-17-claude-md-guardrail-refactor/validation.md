...
|-----|--------------|---------------|
| R1 | Acceptance | `wc -c CLAUDE.md` prints **14000 or less**. Record the exact number. If it exceeds 14000, the slice fails — do not trim a rule into ambiguity to pass; report it instead. (Ceiling amended from 10,000 at `/fix`, 2026-09-17 — see `requirements.md` R1.) |
...
| R8 | Regression | `git diff --name-only "$BASE"...HEAD \| grep -vE '\.md$' \| grep -vE '^(ARTIFACT_INDEX\.md\|app/\(admin\)/staff/runbook/docs\.ts\|specs/2026-09-17-claude-md-guardrail-refactor/.*\.mjs)$'` prints **nothing**. (The first filter drops all markdown; whatever survives must be only the two generated artifacts or this slice's own ledger tooling, and `ARTIFACT_INDEX.md` is already dropped as markdown. Scope amended at `/fix`, 2026-09-17 — see `requirements.md` R8.) |
