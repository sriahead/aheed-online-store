...
| R10 | Unit         | `npx vitest run tests/restock.test.ts` exits 0 and covers yesterday, today, tomorrow and `null`, plus `formatRestockDay("2026-09-28")` containing `Mon` and `28`. Then run the same command with `TZ=Pacific/Pago_Pago` and again with `TZ=Pacific/Kiritimati` prefixed. Both exit 0, which proves the output does not depend on process timezone. `grep -n "process.env\|headers()\|cookies()" lib/restock.ts` prints nothing. |
