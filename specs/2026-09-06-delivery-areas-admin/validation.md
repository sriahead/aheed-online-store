...
| R11 | Integration | Read `lib/delivery-areas-service.ts` and confirm `getPrisma()`, `getPrismaWs()` and `getCurrentVendorId()` are all called inside the exported factory's body, not at module scope. For the *absence* of those calls elsewhere, the authority is `npx vitest run tests/repository-client-injection.test.ts` (exits 0), **not a grep** — that test is AST-based precisely because a repository file legitimately names both functions in prose and in `ReturnType<typeof getPrisma>` type positions. `lib/repositories/delivery-areas.ts` contains three such prose mentions in its header comments, so a grep for `getPrisma()` reports them and the only way to "pass" it would be deleting the rationale. |
