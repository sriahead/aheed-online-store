...
R6. No field added to `prisma/schema.prisma` by this slice is `Json`-typed, and no **hand-written**
    file added or modified by this slice outside `prisma/migrations/` contains `$queryRaw`,
    `$executeRaw` or `$queryRawUnsafe`. `app/(admin)/staff/runbook/docs.ts` is excluded: it is
    generated by `kms:build-index` and embeds documentation prose that discusses raw SQL.
...
R40. The stack container is focusable and its left/right arrow keys move the active card; the
     previous/next controls are `button` elements with distinct accessible names; every focusable
     element in the section shows a visible focus indicator; and every card's text remains in the
     DOM and readable by assistive technology regardless of which card is in front — no card is
     `display: none` or `aria-hidden`.

     **Amended during build (2026-09-19).** As written at `/spec` this required every card to be
     individually tabbable. That specifies an anti-pattern: the cards carry static text, making
     static text focusable is a known screen-reader nuisance, and cards fanned behind the front one
     are visually hidden — so tabbing would put focus on something a sighted keyboard user cannot
     see. Replaced with the standard focusable-container carousel pattern, which keeps the whole
     set readable without creating invisible tab stops. Recorded here and in the build notes rather
...
R49. External review links are created, edited and deactivated from the existing `/staff/storefront`
     page, and **no executable code** added or modified by this slice under `components/` or `lib/`
     contains the literal strings `Google` or `Trustpilot` — no platform name is compiled into a
     conditional, a default, a label or a URL.

     **Clarified during build (2026-09-19).** As written at `/spec` this said "no file … contains",
     which the implementation fails for the right reason: three docstrings state that no platform
     is hardcoded, and the word therefore appears in a comment explaining its own absence. Comments
     are excluded; the rule is about behaviour. Same distinction `tests/panel-refusal-coverage.ts`
     documents and that this slice's own client-choice test had to be fixed for.
