...
| R6  | Unit | `git diff --name-only staging...HEAD \| grep -v "^prisma/migrations/" \| xargs grep -l '\$queryRaw\|\$executeRaw\|\$queryRawUnsafe'` returns nothing; `git diff staging...HEAD -- prisma/schema.prisma \| grep "^+.*Json"` returns nothing. |
...
| R10 | Unit | `npx vitest run tests/customer-feedback-client-choice.test.ts` exits 0 (it asserts the bulk export's parameter type and the facade's client choice). Then `npx tsx scripts/verify-customer-feedback.ts --bulk-approve` against dev prints `R10: PASS bulk approve completed over websocket client` — this is the check that matters, because `updateMany` over the HTTP client throws unconditionally, even on zero rows, so a type-level assertion alone would not catch a wrong runtime client. |
...
| R13 | Security | `scripts/verify-customer-feedback.ts` prints `R13: PASS anonymous submit refused, 0 rows written`. It posts to the action with no session cookie. |
...
| R20 | Unit | Same vitest file covers a whitespace-only comment and a 1001-character comment. Script prints `R20: PASS empty and over-length comments refused, 0 rows written`. |
| R21 | Security | Submit a comment containing `<b>bold</b>` and `[x](https://example.com)`, approve it, then `curl` the landing page. Script prints `R21: PASS comment rendered as escaped literal text`. Confirm by eye that the response body contains `&lt;b&gt;` and no `<b>` element inside the card. |
...
| R36 | E2E | Reject or delete every Aheed row, `curl` the landing page, and confirm neither the section heading nor an empty container appears. Script prints `R36: PASS no section rendered with zero approved rows`. |
...
| R40 | Accessibility | In a real browser at desktop width, Tab through the section: every card takes focus in DOM order with a visible ring, and the previous/next controls are `<button>` with **different** accessible names. Read the names from the accessibility tree, not the visual label — two identically-named controls are indistinguishable to a screen reader and that exact defect is why `HorizontalScroller` requires an `itemLabel`. |
...
| R49 | Unit | Review-link CRUD is present on `/staff/storefront`. `git diff --name-only staging...HEAD \| grep -E "^(components\|lib)/" \| xargs grep -l "Google\|Trustpilot"` returns nothing. |
