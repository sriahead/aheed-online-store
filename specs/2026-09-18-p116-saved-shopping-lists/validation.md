...
| R14 | Unit | `npx vitest run tests/shopping-lists.test.ts` — a case drives `listShoppingLists` against a fake client and asserts the `where` carried both `vendorId` and `userId`, the `orderBy` is `updatedAt` descending, and each returned entry carries `id`, `name`, `itemCount`, `updatedAt`. |
...
| R17 | Unit | `npx vitest run tests/shopping-lists.test.ts` — one case with the count query reporting 20 asserts `createShoppingList` returns `null` and issued no create; a second asserts the same for an empty `items` array. |
...
| R24 | Unit | For each of the five files, `head -1` is `"use server";`, and `grep -c "^export " <file>` returns 1 with that line beginning `export async function`. `npx vitest run tests/server-action-exports.test.ts` exits 0 if that test exists; if it does not, the five `grep` results are the check. |
...
| R37a | Unit | In the rendered HTML from R37's signed-in request, the save-list form's opening tag appears before the add-to-cart form's opening tag and after its closing tag — i.e. the two `form` elements do not overlap. A quick check: pipe the body through `grep -c "<form"` and confirm each `<form` is matched by a `</form>` before the next `<form` begins. In source, `components/cart/ShopYourList.tsx` renders the save form and `components/cart/ListReview.tsx` renders the add form, and neither JSX tree contains the other's `form` element. |
...
| R41 | Unit | `npx vitest run tests/data-rights.test.ts` — a case asserts the export object has a `savedLists` array whose entries carry `name`, `createdAt` and `items` of `rawText`/`quantity`, populated for the given vendor and user. |
...
| R43 | Integration | **live-script** — `verify-saved-lists.ts --erase` seeds a list, calls `eraseVendorData`, and prints `lists deleted: 1 items remaining: 0`, then exits 0. |
