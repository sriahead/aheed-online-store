...
| R2a | Integration  | `npm run sdd:preclear` exits zero on this branch — check the real exit code, not just the printed lines. Its output names exactly one slice, `specs/2026-09-22-kms-enforcement-foundation/`, and does not name `2026-08-21-p8-storefront-branding-webp`, `2026-08-21-view-switcher` or `2026-08-22-ui-polish-docs-integration`. Then confirm no historical spec file was created to achieve that: `git diff --name-status origin/staging...HEAD -- specs/` lists additions only under this slice's own directory. |
