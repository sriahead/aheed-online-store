...
| R20 | Unit, Manual | `grep -n "Proceed to checkout" -A4 -B12 components/cart/CartContents.tsx` shows the control invoking a close on activation rather than being a bare `<Link>`. In a browser: open the drawer, click Proceed to checkout, and confirm the drawer disappears immediately rather than after the checkout page finishes loading. |
