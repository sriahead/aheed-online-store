...
| R7  | Unit | `grep -rnE "text-black/[1-5]0" app components features --include=*.tsx` prints **nothing** and exits 1. Then run `for a in 50 60 70 80 90; do echo "$a $(grep -rho "text-black/$a" app components features --include=*.tsx \| wc -l)"; done` on this branch **and** on `origin/staging`. Expected: `/50` goes from 10 to 0; `/60` rises by exactly that 10 (21 to 31); `/70`, `/80` and `/90` are identical on both sides. A `/60` count that did **not** rise means the ten sites were dropped rather than bumped. |
...
| R10 | Unit | `test -f lib/form-classes.ts` succeeds; `grep -c "use server" lib/form-classes.ts` returns 0; `grep -nE "from \"(next/headers\|@/lib/(db\|auth\|auth-rbac))\"" lib/form-classes.ts` prints nothing. |
...
| R13 | Unit | `grep -rn "focus:outline-none" app components features lib --include=*.tsx --include=*.ts` prints **nothing** and exits 1. |
