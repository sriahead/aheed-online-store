...
| R15 | Regression | `git diff --name-only <base>...HEAD` lists no path beginning `app/`, `components/`, `features/` or `lib/` **except** `app/(admin)/staff/runbook/docs.ts`. Confirm that one file is generated, not hand-edited: run `npm run kms:build-index` and check `git status` reports no further change to it. |
