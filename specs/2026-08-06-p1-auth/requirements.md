...
R2. `lib/auth.ts` exports a configured Better Auth instance: Prisma adapter, email/password
    ...
    on staging vs production domains. No Google/OAuth provider configured in this slice. (Flat
    file, not a `lib/auth/` directory — matches the actual precedent set by `lib/storage.ts` and
    `lib/db.ts` over `docs/repo-structure.md`'s directory sketch, per the Orient step.)
...
R4. `lib/email.ts` exports an `EmailService` port (interface) and a Resend adapter implementing it
    via plain `fetch` against Resend's REST API (no SDK — same Workers bundle-size reasoning as
    `lib/storage.ts`'s `aws4fetch` choice over the AWS SDK). Reads `RESEND_API_KEY` / a from-address
    through `lib/config`. Sending will fail cleanly (not crash the request) until a real API key is
    provided — this slice ships the port/adapter, not a live key.
R4a. **Prerequisite fix, found while validating R2 against the real Workers runtime** (`npm run
    preview`, not `next dev` — see R-validate note below): `lib/db.ts`'s `getPrisma()` cached a
    single Prisma/Neon client across all requests in a warm isolate. Cloudflare Workers forbids
    reusing I/O objects across request boundaries; rapid sequential requests failed ~1-in-3 times
    with `"Cannot perform I/O on behalf of a different request"` — a pre-existing risk since M0
    (affecting `/api/health` too), just never caught because validation never hammered it with
    back-to-back requests. Fix: `getPrisma()` no longer caches across requests — a fresh
    Prisma/Neon client is constructed per call, matching Neon's own recommended pattern for
    serverless/edge (its HTTP driver is cheap to instantiate, not a persistent connection to
    preserve). `getAuth()` inherits the fix by depending on `getPrisma()`.
