...
| R6  | Unit         | `npx vitest run tests/product-card.test.tsx` exits 0, with a `// @vitest-environment jsdom` case that renders `ProductCard` for a product with a `primaryImage`, fires an `error` event on the rendered `<img>` (`fireEvent.error`), and asserts the `<img>` is gone and an element carrying `bg-surface-muted` is present. |
...
| R12 | E2E          | Under `npm run preview`, `/staff/products` renders a checkbox for the Open Food Facts source, checked on first load. Uncheck it, click **Auto-fill Missing Images**, and confirm in the local Worker log that no Open Food Facts request was made — query `POST http://127.0.0.1:8787/cdn-cgi/local/explorer/api/local/observability/query` with `{"sql": "SELECT ts_ms, level, message FROM logs ORDER BY ts_ms DESC LIMIT 50"}` (the pattern `CLAUDE.md` records for reading preview logs). |
