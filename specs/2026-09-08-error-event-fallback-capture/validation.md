...
| R2  | Unit         | `grep -nE '^import' lib/error-event-fallback.ts` shows no line containing `@prisma/client`, `@prisma/client/wasm` or `@/lib/db`. Then `grep -nE 'getPrisma[A-Za-z]*\(' lib/error-event-fallback.ts` prints nothing (exit 1). Note the deliberate absence of an alternation pattern here: a `\|` written inside a table cell is a **literal** pipe character to `grep -E`, not alternation, so such a command silently searches for the wrong string. |
...
| R7  | Unit         | `grep -nE 'readEnv\(' lib/error-event-fallback.ts` shows a call with `"DATABASE_URL"`; `grep -nE 'getEnv\(' lib/error-event-fallback.ts` prints nothing (exit 1).                                                                                                                                                          |
