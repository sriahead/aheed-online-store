...
| R18 | Integration | The migration was created with `npx prisma migrate dev --create-only`. `cat prisma/migrations/20260909140355_p9_3_product_created_at_index/migration.sql` shows the `CREATE INDEX`. For the drops, count **executable** statements only — `grep -cE '^\s*DROP INDEX' prisma/migrations/20260909140355_p9_3_product_created_at_index/migration.sql` returns `0`. **Do not use a bare `grep -c "DROP INDEX"`**: it returns `4` on the correct file, because the migration's own comment block quotes the three statements it removed and names the problem. (Corrected during Build after the bare form did exactly that — the same class of self-matching grep `CLAUDE.md` records for `"use server"` and `focus:outline-none`.) `build-notes.md` records that the drops were generated and removed by hand before anything was applied. |
