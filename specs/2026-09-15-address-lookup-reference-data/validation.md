...
8. **Reference data lives in its OWN Neon project**, `uk-location-reference` (branch
   `uk-location-reference-staging` for this validation), reached through `UK_LOCATION_REF_DATABASE_URL`
   (pooled, runtime) and `UK_LOCATION_REF_DIRECT_URL` (direct, migrations and sync). These are
...
9. **Never point staging at production reference data.** Check the host in `UK_LOCATION_REF_DIRECT_URL`
...
Read each log file rather than piping it. Coverage defaults to `UK_LOCATION_REF_POSTCODE_AREAS`
...
|-----|--------------|---------------|
| R1 | Unit | `grep -n "datasource\|UK_LOCATION_REF_DATABASE_URL\|UK_LOCATION_REF_DIRECT_URL\|output" prisma/reference/schema.prisma` shows a datasource reading both reference URLs and a generator with its own `output`. `grep -n "model ReferenceDataset" -A 18 prisma/reference/schema.prisma` lists every field named in R1. |
...
| R5b | Performance | After the MK+RG imports, run the size measurement against `UK_LOCATION_REF_DIRECT_URL` (`pg_total_relation_size` per table) and confirm `PostcodeReference` bytes divided by its row count is under 120. Record both table sizes and the per-row figure in `build-notes.md`. |
...
| R8 | Integration | `git diff --stat origin/staging -- prisma/migrations/` shows this slice's new directories (two, per the v1→v2 pivot — `git status --short` alone shows nothing once committed). `grep -rn "DROP INDEX" prisma/migrations/<each-new-dir>/migration.sql` returns no line naming `Order_orderNumber_trgm_idx`, `Order_guestEmail_trgm_idx` or `User_email_trgm_idx`. `npx prisma migrate status` prints that the database schema is up to date. |
| R9 | Unit | `npx vitest run tests/postcode-normalisation.test.ts` exits 0, including cases asserting `"mk9 2nw"`, `" MK9  2NW "` and `"MK92NW"` all normalise to `"MK92NW"` and that `"MK92NW"` formats to `"MK9 1AA"`. `grep -nE "^import .*(@prisma/client\|next/headers\|node:fs)" lib/postcode-normalisation.ts` returns nothing. |
...
| R13 | Integration | **Corrected at Document (final) 2026-09-16** — no dedicated `tests/reference-source-code-point.test.ts` exists; `discoverLatest` is exercised only live, not by a unit double. Confirm live and keyless: `curl -s https://api.os.uk/downloads/v1/products/CodePointOpen` returns a JSON body containing `"version"`, with no credential sent. `grep -niE "authorization\|api[-_]?key" lib/reference-data/sources/code-point.ts` returns nothing. `npx vitest run tests/reference-sync-integrity.test.ts` covers the same file's pure `applyPostcodeRecords`. |
...
| R25a | E2E | Against the dev database, delete every row from `PostcodeReference` and `PlaceReference` (they are rebuildable cache data by definition — this is the clean-environment test). With `npm run preview` NOT running, run both sync commands and redirect each to a log file; read the logs and confirm both report a non-zero `inserted` count and status `SUCCEEDED`, and that row counts are non-zero afterwards. Confirm `gh workflow run sync-reference-data.yml` is accepted without any deploy workflow running. *Confirmed live at Build against the then-empty tables (see `build-notes.md`'s "Live evidence already gathered": `40,031` Code-Point rows, `23,472` Open Names records); not re-executed at `/validate` 2026-09-15 — deliberately, given the cost of deleting real rows from the shared dev/staging reference database to re-prove an already-demonstrated capability. `gh workflow run` could not be confirmed pre-merge either: `workflow_dispatch` only registers once a workflow file lands on a branch GitHub has synced, which this branch was not until `/ship`.* |
...
| R27 | E2E | **The coverage distinction.** Request `/api/address/lookup?postcode=EH1+1YZ` — a real postcode in an area deliberately NOT materialised. Confirm `status` is `UNVERIFIED`, **not** `INVALID`. Repeat for `SW1A 1AA`. Then confirm `MK9 1AA` reports `VALID`, proving the two are genuinely distinguished rather than everything being unverified. |
...
| R27b | Resilience | Point `UK_LOCATION_REF_DATABASE_URL` at an unreachable host, restart `npm run preview`, and request the lookup for `MK9 1AA`. Confirm HTTP 200 with `status` `UNVERIFIED` — not a 500, not `INVALID` — and that checkout still accepts a manually entered address. Restore the URL afterwards. This failure mode is the whole reason the state exists once reference data sits behind its own database. *Confirmed live at Build (see `build-notes.md`'s "Live evidence already gathered": `MK9 1AA` → `UNVERIFIED deliverable=true`, `MK99 9ZZ` → `UNVERIFIED deliverable=false`); not re-run at `/validate` 2026-09-15, since it needs its own `.dev.vars` edit and full preview restart. `lib/reference/postcode-reference-service.ts`'s unconditional try/catch → `unverified()` fallback (read at `/validate`) is the code-level guarantee this row exercises live.* |
...
| R30 | Unit | **Corrected at Document (final) 2026-09-16** — no `tests/address-lookup-service.test.ts` exists; `lookupAddress`/`lookupPostcodeReference` resolve their own Prisma client and are not unit-tested directly. Confirmed instead by reading `lib/reference/postcode-reference-service.ts`: `status`/`areaCovered`/`reference` are all set before `findPlacesNear` runs, so an empty `PlaceReference` result can only change `location`, never validity or (via `lib/delivery-eligibility.ts`) deliverability — and live-confirmed at `/validate` (`MK9 1AD` returned `DELIVERABLE` with `streetSuggestions: []`). |
| R31 | Unit | `npx vitest run tests/places-repository.test.ts` — `rankNearbyStreets` asserts a road at 240 metres is included and one at 260 metres is excluded, and that a road in a different `postcodeDistrict` is excluded regardless of distance. |
...
| R33 | Unit + E2E | Same test file asserts an empty list in all three suppression cases: no qualifying road, a nearest road beyond 200 m, and more than 25 distinct names within 250 m. **Then confirm against real Milton Keynes data** under `npm run preview`, with both datasets synced: request the lookup for a dense central-MK postcode (a `MK9` grid-square unit, where many named boulevards sit within 250 m) and confirm `location.streetSuggestions` is `[]` rather than an arbitrary pick; request it for a sparser outlying unit (e.g. in `MK17`) and confirm any suggestion returned is a street genuinely at that location. Record both postcodes and both responses in the build notes — a misleading hint is the failure this row exists to catch, and it is only visible against real data. *Confirmed at `/validate` 2026-09-16: `MK9 1AD` (dense central MK) returned `[]`; `MK17 8NL` (sparse outlying) returned `["Dene Close","Downham Road","Aspley Hill"]`, and its coordinates independently matched postcodes.io to five decimal places.* |
...
| R36 | E2E | Under `npm run preview`, for each of `MK9 1AA` (covered), `RG30 2BE` (covered) and `EH1 1YZ` (not covered), `curl -s "http://127.0.0.1:8787/api/address/lookup?postcode=…" -H "Host: <resolved host>"` returns a body whose `addresses` is `[]`. |
| R37 | Integration | The same responses contain all six keys, with `manualEntryAvailable` true in each. |
| R37a | Integration | Pipe the `MK9 1AA` response through `node -e` and print `Object.keys(body.location).sort()`; it equals `county,district,latitude,longitude,streetSuggestions,town`. Confirm `latitude` is near `52.0` and `longitude` near `-0.76` (WGS84 degrees, not a six-figure grid value), and that every entry of `streetSuggestions` is a plain string. |
...
| R41 | E2E | Under `npm run preview`, on `/checkout`, enter `MK9 1AA` and trigger the lookup; the `city` input is populated and the `line1` and `line2` inputs are empty and editable. |
