...
| R27b | Resilience | Point `UK_LOCATION_REF_DATABASE_URL` at an unreachable host, restart `npm run preview`, and request the lookup for `MK9 1AA`. Confirm HTTP 200 with `status` `UNVERIFIED` — not a 500, not `INVALID` — and that checkout still accepts a manually entered address. Restore the URL afterwards. This failure mode is the whole reason the state exists once reference data sits behind its own database. |
