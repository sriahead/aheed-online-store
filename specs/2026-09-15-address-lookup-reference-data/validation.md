...
| R49 | Unit | `ls lib/postcodes-api.ts features/checkout/postcode-lookup.ts tests/postcodes-api.test.ts` reports all three missing, and `grep -rn "postcodes-api\|postcode-lookup" app components features lib tests --include=*.ts --include=*.tsx` returns nothing. |
