...
| Req | How to verify |
|-----|---------------|
| R1  | `awk '/\*\*P8.6 — Storefront discovery/,/\*\*P8.7 — Fulfilment/' specs/roadmap.md \| grep -o '#[0-9]\+' \| sort -u -V` — the printed set includes #394, #395, #396, #397, #400, #404, #405 and #406. |
| R2  | `awk '/\*\*P8.7 — Fulfilment/,/^## /' specs/roadmap.md \| grep -o '#[0-9]\+' \| sort -u -V` — the printed set includes #397, #398, #399, #400, #401 and #402. |
| R3  | `awk '/\*\*P8.2 — Launch/,/\*\*P8.3 —/' specs/roadmap.md` — read the output. It names #398, #403, #407 and #397 as the pre-launch set, and says in prose that the set is sequenced by this slice and built by a separate later slice under its own issue. Judgement row: confirm the prose actually says a later slice builds it, not merely that the items are pre-launch. |
