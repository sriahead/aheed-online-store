...
| R13 | Unit | `npx vitest run tests/config.test.ts` (or the suite covering `lib/config.ts`) passes a case asserting the accessor throws when the token is absent under `NODE_ENV=production` and does not throw otherwise. Confirm the accessor uses `readEnv`, not `process.env` directly: `grep -n 'readEnv("JOB_INVOCATION_TOKEN")' lib/config.ts`. |
