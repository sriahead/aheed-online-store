...
| R23 | Security | With `npm run preview` running, comment out `STRIPE_SECRET_KEY` in **both `.env` and `.dev.vars`** — `.env`'s value alone survives into the built Worker's `process.env` (`next build` bakes it in) and `readEnv()` falls back to it, so `.dev.vars` alone does not simulate "unset"; restart preview, `curl -X POST` with a valid token returns `503`. Confirm via the log query that no order transition occurred. Restore both and restart. |
