...
| R4  | Integration | `git diff origin/staging -- package-lock.json` prints **nothing** (empty diff). If it prints anything, inspect it: any change to a `"version"` field for the three pinned packages fails this row. |
...
| R8  | Unit | `sed -n '/## Dependency & version discipline/,/^## /p' CLAUDE.md > /tmp/dep.txt` then `grep -c "0.10.4" /tmp/dep.txt` prints `0`, `grep -c "must not be used" /tmp/dep.txt` prints `0`, and `grep -c "1.1.0" /tmp/dep.txt` and `grep -c "7.9.1" /tmp/dep.txt` each print at least `1`. |
