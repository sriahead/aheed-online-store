...
R1. Front-matter is added to 19 files: `CLAUDE.md`; `docs/onboarding.md`,
    ...
    `2026-08-06-p0-foundation/plan.md`, `2026-08-06-p1-auth/requirements.md`.
...
R3. A sibling file within an already-covered slice folder (e.g. a slice's `requirements.md` when
    ...
    issue #25 for why).
R4. `npm run kms:validate` reports 0 invalid front-matter and exactly 20 valid (19 new + the
    already-existing `specs/2026-08-06-kms/plan.md`).
R5. `npm run kms:build-index` regenerates `ARTIFACT_INDEX.md` with 20 rows, correctly grouped by
