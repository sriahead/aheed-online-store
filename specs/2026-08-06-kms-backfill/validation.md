...
| Req | How to verify |
|-----|---------------|
| R1  | `git diff --stat` on the branch — exactly the 19 listed files touched (plus generated `ARTIFACT_INDEX.md`). |
...
| R4  | `npm run kms:validate` output: `valid front-matter: 20`, `invalid front-matter (failing): 0`. |
| R5  | `npm run kms:build-index`; `ARTIFACT_INDEX.md`'s Track 1 table has 20 rows; `grep -c '^|' ARTIFACT_INDEX.md` sanity-checks the row count. |
