...
| R29 | Integration | `grep -c 'repo.matchListTerms(' features/cart/match-list.ts` prints `1`, and `git diff origin/staging -- features/cart/add-list-to-cart.ts` prints nothing. (Corrected at Build: this row originally grepped the bare name `matchListTerms`, which prints `2` because the file names the function in a slice-3 comment as well as calling it — it printed `2` before this slice too, so the row could never have passed. The requirement is unchanged; only the step that checks it was wrong. Same class as the two measurement errors caught at Build in slice 1.) |
