...
| R5  | `grep -n "note" lib/order-status.ts` prints nothing. Read `getForUser`'s `statusEvents` select: `status` and `createdAt` only. Then write a note directly into the DB (`UPDATE "OrderStatusEvent" SET note='INTERNAL-DO-NOT-SHOW' WHERE "orderId" = <O1>`) and confirm `INTERNAL-DO-NOT-SHOW` does not appear in the body of `GET /account/orders/{O1}`. |
...
| R9  | Run the preview with Prisma query logging on (or count queries in a driver script calling `listForUser(userId, { take: 10 })` against 12 fixture orders): exactly **one** `SELECT` against `"Order"` for the page, with items arriving via the nested select — not one query per order. |
...
| R27 | `grep -n "Todo\|do not exist yet" specs/sdd-workflow.md` returns nothing in the delivery-board section, and `git diff` shows its `version`/`updated` front-matter bumped. |
