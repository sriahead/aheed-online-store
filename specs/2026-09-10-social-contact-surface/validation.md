...
| R19 | Integration | `grep -n 'lg:items-stretch' 'app/(landing)/page.tsx'` prints one line and `grep -c 'lg:items-center' 'app/(landing)/page.tsx'` prints `0`. `grep -c 'h-full' components/layout/DepartmentHero.tsx` prints at least `3` (root, track, slide). Then confirm Tailwind actually **generated** the variants, because an ungenerated class is inert: in the built CSS (`find .next/static/css -name '*.css'`), `grep -cF 'lg\:items-stretch'` and `grep -cF 'lg\:h-full'` each print `1` — use `grep -F`, the CSS contains a literal backslash and a normal regex grep silently returns `0` for all of them, including classes that demonstrably work. Finally, **look at the page**: at a desktop width the panel fills the full height of the hero's right column with the section's own padding still visible around it, and at tablet/mobile width the hero is a single column with the panel unchanged. `npx vitest run tests/department-hero.test.tsx` passes (16 tests). |
