...
| R8 | Integration & Accessibility | (a) In `home.html`, find the single container holding all three anchors (`grep -oE '<div class="fixed bottom-[^"]*"' home.html` prints one match) and confirm its `bottom-` utility is numerically greater than `6` and its `sm:bottom-` greater than `8`. Cross-check `grep -n 'fixed bottom-6 right-6' components/cart/CartDrawerShell.tsx` still prints the cart button's line — if the cart button moved, this row's premise changed. Confirm the three anchors are inside that one container, not positioned individually. (b) In `components/layout/FloatingContact.tsx`, confirm the hidden state applies a translate/opacity change and `focus-within:` counterparts restore it, and that the component never returns early in a way that unmounts the container while values exist. In a browser at `npm run preview`: scroll down — the cluster slides away; scroll up — it returns; press Tab until a control inside it takes focus while hidden — it becomes visible. (c) `npx vitest run tests/motion-reduce-coverage.test.ts` passes, and the container carries `motion-reduce:transition-none`. (d) `grep -nE '#[0-9a-fA-F]{3,8}' components/layout/FloatingContact.tsx` prints `0` lines. Grep the source, not `home.html`: a rendered page legitimately embeds the vendor's own brand hex in `brandStyle()`'s inline custom properties, so a live-HTML hex grep cannot distinguish a hardcoded literal from a vendor primitive (`#631`). |
| R8a | Unit | `grep -n -A3 'window.addEventListener("scroll"' components/layout/FloatingContact.tsx` — the enclosing `useEffect`'s dependency array is `[]`, and `grep -n 'useRef' components/layout/FloatingContact.tsx` shows the previous offset held in a ref. A dependency array containing the visibility state is the cart-drawer bug shape from `CLAUDE.md`'s React section: the effect would tear down and re-add its own listener on every toggle. |
| R9 | E2E | Fetch the **second vendor's** storefront, whose three columns are still `null` after seeding: `curl -s -H "Host: <srimart-host>" http://127.0.0.1:8787/ > srimart.html`. Then `grep -c 'wa\.me' srimart.html`, `grep -ci 'facebook' srimart.html` and `grep -ci 'instagram' srimart.html` each print `0`, and the floating container itself is absent (`grep -c 'flex-col-reverse' srimart.html` prints `0`) — the component returns `null` when all three values are, so an empty container must not ship either. Use the second vendor rather than Aheed-with-nulls: a live-HTML absence check is only meaningful against a vendor whose own data cannot coincidentally supply the string. |
