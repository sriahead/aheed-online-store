...
| R5 | Unit | Same scratch script: the WhatsApp parser accepts `447700900123`, rejects `+447700900123`, `44 7700 900123` and `44-7700-900123`, rejects `123456` (6 digits) and a 16-digit value, and returns success with `null` for `""`. |
| R6 | Security & Accessibility | With Aheed's three fields set (see R13), fetch the storefront home under `npm run preview` using the host from step 1: `curl -s -H "Host: <aheed-host>" http://127.0.0.1:8787/ > home.html`. Then `grep -oE '<a[^>]*facebook[^>]*>' home.html` and the Instagram equivalent each print one match, and each printed tag contains `noopener`, `noreferrer`, and an `aria-label` (or visible text) containing both the vendor name and the network name. Confirm the accessible name is not an unlabelled icon by checking the tag is not `aria-hidden`. |
...
| R8 | Integration & Accessibility | (a) `grep -oE '<a[^>]*wa\.me[^>]*>' home.html` — read the `class` attribute on that tag and confirm its `bottom-` utility is numerically greater than `6` and its `sm:bottom-` utility greater than `8`. Cross-check against `grep -n 'fixed bottom-6 right-6' components/cart/CartDrawerShell.tsx`, which must still print the cart button's line — if the cart button moved, this row's premise changed and the comparison must be redone against its new value. (b) `npx vitest run tests/motion-reduce-coverage.test.ts` passes. (c) Grep the WhatsApp component's **source** (not the rendered HTML) for a hex literal — `grep -nE '#[0-9a-fA-F]{3,8}' <the component file>` prints `0` lines. Grep the source, not `home.html`: a rendered page legitimately embeds the vendor's own brand hex in `brandStyle()`'s inline custom properties, so a live-HTML hex grep cannot distinguish a hardcoded literal from a vendor primitive (`#631`). |
| R9 | E2E | Fetch the **second vendor's** storefront, whose three columns are still `null` after seeding: `curl -s -H "Host: <srimart-host>" http://127.0.0.1:8787/ > srimart.html`. Then `grep -c 'wa\.me' srimart.html`, `grep -ci 'facebook' srimart.html` and `grep -ci 'instagram' srimart.html` each print `0`. Use the second vendor rather than Aheed-with-nulls: a live-HTML absence check is only meaningful against a vendor whose own data cannot coincidentally supply the string. |
