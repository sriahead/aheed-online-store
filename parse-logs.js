const fs = require('fs');
const logFile = 'C:\\Users\\User\\.gemini\\antigravity-cli\\brain\\b16fea92-8659-44b0-a36d-f37f0e8cf923\\.system_generated\\tasks\\task-1496.log';
...
for (const match of matches) {
  try {
    ...
    if (logs.length > 0) {
      logs.forEach(l => {
        if (l.level === 'error' || l.message.some(m => typeof m === 'string' && (m.includes('Error') || m.includes('Exception') || m.includes('441')))) {
          console.log(l);
        }
      });
    }
