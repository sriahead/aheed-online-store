...
- **Net content for the generated demo catalogue** (issue `#877`, split from `#697`; `specs/2026-09-23-p877-generated-net-content/`). *Spec approved 2026-09-24; build pending — this entry is completed at `/build-notes`.* The seed's `gen-` products gain net content for their eight weight and volume packs, from a fixed lookup table rather than any parsing, plus a backfill for rows already in a database, so `#397`'s pack-size facet and `#398`'s derived unit price become visible in dev. No real or curated product changes, and nothing is written outside dev. `#697` stays open for real products.
