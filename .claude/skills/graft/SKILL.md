...
- A node's `covers:` list already gives exact `file:line` for every symbol, so
  cite straight from it. The spans are generated from source; don't re-open or
  re-grep files merely to repeat information you already have. When a claim is
  used as evidence — implementation correctness, validation, security,
  behaviour, architecture decisions, or durable documentation — verify it
  against the authoritative source: the file itself, an approved spec or ADR, a
  test, or runtime output.
...
- Act on the answer for navigation, scoping and impact analysis. Reach for a
  second tool only when the first genuinely fell short: weak hits, a truncated
  span, or a need to be exhaustive. Graft cards are structural projections,
  rebuilt at the end of a turn, and may lag the working tree.
...
## Output handling
Each retrieval tool opens its output with a `[graft] tokens saved ≈ N` line, and
the statusline keeps a running session total. These are metrics for our own
benchmarking — do **not** add a per-turn savings tally, or any other
graft-branded summary line, to your replies. Answer as you normally would.
