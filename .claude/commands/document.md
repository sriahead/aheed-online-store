...
it. Verified permanent documentation supersedes an incorrect build-time claim, but it does not
replace `build-notes.md`'s slice-local record.
...
3. Run the **final-state handoff reconciliation**: did Validate/Fix/Ship materially change the
   overall project state a fresh model needs for recovery? If yes, update authoritative
   documentation where appropriate first, then reconcile `docs/model-handoff.md` with the final
   shipped state. Remove or replace stale information rather than appending history, and reference
   authority rather than copying it. If nothing material changed, leave the handoff untouched. This
   does not turn Build Notes into final documentation or the handoff into another changelog.
4. Update `specs/roadmap.md` — progress, plus a closure note in its change log if this closed out a
   ...
   existed.
5. **Reconcile the delivery board** (status-and-priority view, never scope): every issue for this slice
...
6. Anything still deferred or incomplete is a tracked GitHub issue — never a comment or a memory.
7. Record anything the loop itself taught (a trap worth encoding in `specs/sdd-workflow.md` or
   `CLAUDE.md`) while it's still cheap to write down.
8. Later phases (P7 compliance, P8 handover) need compliance reports / a handover pack per their own
   specs — call that out explicitly in that phase's `requirements.md` rather than assuming this
   stage covers it by default.
9. **If this slice closed out a milestone, the loop is not finished here.** Run `/discover`, then
   `/learn`, then the **business case review**, before the model switch and `/clear` — see
   **Milestone close** in `specs/sdd-workflow.md`. The roadmap closure row you just wrote records
   *what* closed; those three stages are what turn the milestone into durable knowledge
   (`docs/research/discovery-log.md`, `docs/research/milestone-retrospectives.md` and
   `docs/business-analysis/business-case.md`) instead of context that dies at the next `/clear`.
   None produces implementation scope — findings reach the roadmap through `/propose`.

   **Expect step 4's `npm run sdd:audit` to report the business case as due the moment you write a
   closure row.** That is the check working: the roadmap now says a milestone closed and the
   business case has not yet been reviewed against it. It clears when the review lands, and the
   milestone close is not finished until `sdd:audit` exits 0 again.
