...
// npm's default global prefix on Windows is %APPDATA%\npm. Derived, not baked, so the
// file is portable; null elsewhere (or if APPDATA is unset), which `best()` skips —
// resolution then falls through to fromPkg()/`npm root -g` exactly as before.
const BAKED = process.env.APPDATA
