...
_Generated from front-matter across the repo. Last build: `2026-08-09T19:01:47.195Z` · commit `04a9789` · `38` artifacts._
...
| [ADR-004 slice 3c — data-driven auth cookie scoping (plan)](specs/2026-08-09-multitenancy-slice3c-auth-cookie-scoping/plan.md) | spec | 1.0.0 | 2026-08-09 | draft | internal | Make Better Auth's baseURL, trustedOrigins and cookie domain data-driven per request from the resolved host + VendorDomain table. Isolated (host-only) sessions by default; a config-gated AUTH_COOKIE_FAMILY_DOMAIN arms future subdomain-family SSO, off today. |
| [System Architecture — Aheed Online Store](specs/architecture.md) | doc | 1.6.0 | 2026-08-09 | approved | internal | The technical source of truth for infrastructure and Clean Architecture layering — Cloudflare Workers + Neon + S3-compatible storage, vendor-agnostic and multi-tenant (vendor-scoped) by design. |
...
| [ADR-004 — Multi-Tenancy (DB-driven vendors, regions & branding)](specs/decisions/ADR-004-multi-tenancy.md) | adr | 1.1.0 | 2026-08-09 | approved | internal | Evolve from single-vendor to a multi-tenant platform where vendors, regions, locations, delivery areas, and branding come from the database, sharing one business-logic and data layer. Row-level vendorId isolation, subdomain resolution, isolated-by-default auth (family SSO config-gated). |
