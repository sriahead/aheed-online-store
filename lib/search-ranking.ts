 ...
 */
function tierOf(candidate: SearchCandidate, terms: readonly string[], joined: string): number {
  ...
  if (allTermsInName) return candidate.inStock ? 1 : 2;
...
export function rankSearchCandidates<T extends SearchCandidate>(
  ...
  for (const candidate of candidates) {
    tiers.set(candidate, tierOf(candidate, terms, joined));
  }
