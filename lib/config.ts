...
const emailSchema = z
  .object({
    // Optional in dev/CI: no key means email sending degrades (logs, doesn't crash the
    // request) until the human provisions one.
    // P9.1 (#430) — Enforced in production via superRefine to prevent silent failures.
